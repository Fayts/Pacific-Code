...
export default async function CustomerPage({
  ...
  const [{ data: customer }, { data: bookingsData }] = await Promise.all([
    supabase
      .from("customers")
      .select("*")
      .eq("id", id)
    ...
    supabase
      .from("bookings")
      .select(
        "id, booking_number, status, start_at, end_at, total_amount, booking_items(quantity, equipment_items(name))"
      )
      .eq("organization_id", context.organization.id)
      .eq("customer_id", id)
  ...
  const history = bookings.map((booking) => ({
    ...
    materials:
      booking.booking_items
        .map((item) => {
          const equipmentName = item.equipment_items?.name ?? "Matériel supprimé";
